...
HRESULT
encoding_convt_cbsize(const char *srccode, const char *dstcode, const char *src, size_t src_cbsize,
  size_t *dst_cbsize)
{
  iconv_t cd;
  char buffer[0x100] = {};
...
HRESULT
encoding_convt(const char *srccode, const char *dstcode, const char *src, size_t src_cbsize, char *dst,
  size_t *dst_cbsize)
{
  iconv_t cd;
  char *inbuf = (char *)(src);
...
HRESULT
utf32le_2_mbcs_posix2(const void *src, size_t arg1, void *dst, size_t *dst_size)
{
  ...
  if (dst)
    z = wcstombs((char *)dst, (const wchar_t *)src, *dst_size);
  else
    z = wcstombs(NULL, (const wchar_t *)src, 0);
...
mbcs_2_utf32le_posix2(const void *src, size_t src_size, void *dst, size_t *dst_size)
{
  ...
  setlocale(LC_ALL, "zh_CN.GBK");
  if (dst)
    z = mbstowcs((wchar_t *)dst, (const char *)src, *dst_size >> 1);
  else
    z = mbstowcs(NULL, (const char *)src, 0);
  if (z == (size_t)(-1))
    hr = E_FAIL;
  else
    *dst_size = z << 1;
